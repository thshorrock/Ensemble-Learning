...
namespace ICR{
...

  {
	...
	calculator_context<T,to_node> M0; // The average means (inverted): < expr^-1(x_i) >
	calculator_context<T,to_node> M1; // The average of squares: < expr^-1(x_i)^2 >
	...
	//zero is the results node.
      calculator_context<T,0> M0; //All the first moments  (the <x>'s of every element in expr)
      calculator_context<T,0> M0_squared; //All the first moments  (the <x>^2's of every element in expr)
      calculator_context<T,0> M1;//The second moment (the <x^2> of every element of expression)
	for(size_t i=0;i<M.size();++i){
...
ICR::EnsembleLearning::RectifiedGaussian<T>::CalcSample(moments_parameter Mean,
					   moments_parameter Precision) 
{
