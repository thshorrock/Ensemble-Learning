...
namespace ICR{
  namespace ICA{
    ...
    public:
      ...
      ForwardingNode(const size_t moment_size = 2); //mostly two, but variable for Discrete model
...
ICR::ICA::ForwardingNode<Model,T>::ForwardingNode(const size_t moment_size) 
...
template<class Model,class T>
void 
ICR::ICA::ForwardingNode<Model,T>::Iterate(Coster& C)
{
  //EVALUATE THE COST
