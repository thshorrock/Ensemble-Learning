...
namespace ICR{
  namespace ICA{
    ...
    class ComponentNode : public VariableNode<T>, 
		       public LearningNode<T>,
		       public CanMakeMixture<T>
    {
    public:
      
      ComponentNode(const size_t moment_size = 2); //mostly two, but variable for Discrete model
...
ICR::ICA::ComponentNode<Model,T>::ComponentNode(const size_t moment_size) 
...
inline 
void
ICR::ICA::ComponentNode<Model,T>::SetParentFactor(FactorNode<T>* f)
{
...
inline
void
ICR::ICA::ComponentNode<Model,T>::AddChildFactor(FactorNode<T>* f)
{ 
...
inline
ICR::ICA::Moments<T>
ICR::ICA::ComponentNode<Model,T>::GetMoments() const
{
...
inline  
double
ICR::ICA::ComponentNode<Model,T>::GetAvLog() const 
{
  /*This value is update once in update stage,
   * and called in a Factor update stage.
   * These never overlap and so this is thead safe
...
// T
// ICR::ICA::ComponentNode<Model,T>::GetNormalisation() const 
// {
...
template<class Model,class T>
void 
ICR::ICA::ComponentNode<Model,T>::Iterate(Coster& C)
{
  //EVALUATE THE COST
  ...
  const NaturalParameters<T> weight = m_weights->GetNaturalNot(this);
  //Add it to the total
  NP += ParentNP*weight; //total
