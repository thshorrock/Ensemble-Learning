...
namespace ICR{
  namespace ICA{
    ...
    public:
      ...
      DeterministicFactor( std::vector< VariableNode<T>*> ParentA,  
			   std::vector< VariableNode<T>*> ParentB,  
	...
	for(size_t i=0;i<ParentA.size();++i){
	  ParentA[i]->AddChildFactor(this);
	  ParentB[i]->AddChildFactor(this);
	}
    ...
    public:
      ...
      DeterministicFactor( VariableNode<T>* ParentA,  VariableNode<T>* ParentB,  DeterministicNode<Model<T>,T>* Child)
	: m_parentA_node(ParentA),
	  m_parentB_node(ParentB),
	  m_child_node(Child)
      {
